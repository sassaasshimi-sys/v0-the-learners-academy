  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 className="font-serif text-3xl font-normal text-foreground">Content Audit & Registry</h1>
        ...
        <div className="flex items-center gap-2">
          {pendingCount > 0 && (
            <Badge variant="destructive" className="animate-pulse gap-1.5 px-3 py-1 bg-destructive/10 text-destructive border-destructive/20 h-9 rounded-xl">
        ...
        <div className="p-4 flex flex-col gap-4 md:flex-row md:items-center">
          <div className="relative flex-1">
            ...
            <Input 
              ...
              className="pl-9 h-11 bg-background/50 border-primary/5 rounded-xl text-sm" 
            />
          </div>
          <div className="flex items-center gap-2">
             <Tabs value={filterMode} onValueChange={(v) => setFilterMode(v as any)} className="bg-muted/30 p-1 rounded-xl">
               <TabsList className="bg-transparent h-9 border-none p-0">
                  <TabsTrigger value="all" className="h-7 px-4 rounded-lg data-[state=active]:bg-card data-[state=active]:shadow-sm text-[10px] uppercase tracking-widest">All</TabsTrigger>
                  <TabsTrigger value="pending" className="h-7 px-4 rounded-lg data-[state=active]:bg-card data-[state=active]:shadow-sm text-[10px] uppercase tracking-widest">Pending</TabsTrigger>
                  <TabsTrigger value="approved" className="h-7 px-4 rounded-lg data-[state=active]:bg-card data-[state=active]:shadow-sm text-[10px] uppercase tracking-widest">Approved</TabsTrigger>
               </TabsList>
             </Tabs>
          </div>
        </div>
        ...
        <AnimatePresence mode="popLayout" initial={false}>
          {filteredQuestions.length === 0 ? (
            ...
            filteredQuestions?.map((q) => (
              ...
              >
                <Card className={`group overflow-hidden bg-card/40 backdrop-blur-md shadow-premium rounded-[1.5rem] border-none transition-premium ring-1 ${q.isApproved ? 'ring-primary/5' : 'ring-warning/30 bg-warning/[0.01]'}`}>
                    ...
                    <div className="p-6 flex-1 flex flex-col md:flex-row gap-6">
                      <div className="flex-1 space-y-3">
                        <div className="flex items-center gap-2 flex-wrap">
                          <Badge variant="outline" className={`text-[9px] px-2 h-5 font-bold uppercase tracking-widest border-none ${TYPE_BADGE_COLORS[q.type] || ''}`}>
                            {q.type}
                          </Badge>
                          <Badge variant="secondary" className="text-[9px] px-2 h-5 font-normal uppercase tracking-widest bg-muted/40">
                            {q.phase}
                          </Badge>
                          <Badge variant="outline" className="text-[9px] px-2 h-5 font-normal uppercase tracking-widest border-primary/10">
                            {q.category}
                          </Badge>
                          {!q.isApproved && (
                            <Badge className="bg-warning text-warning-foreground text-[8px] font-bold uppercase tracking-tighter px-1.5 h-4">
                              Awaiting Audit
                            </Badge>
                          )}
                        </div>
                        ...
                        <p className="text-lg leading-relaxed font-sans font-normal text-foreground/90">
                          {q.content}
                          {q.type === 'Fill in the Blanks' && <span className="ml-2 font-serif text-primary border-b-2 border-primary/30 px-2 italic">{q.correctAnswer}</span>}
                        </p>
                      ...
                      <div className="flex flex-row md:flex-col items-center justify-between md:justify-center gap-4 md:border-l border-primary/5 md:pl-8">
                        <div className="flex flex-col items-center gap-1.5">
                          <span className={`text-[9px] uppercase tracking-widest font-bold ${q.isApproved ? 'text-success' : 'text-warning'}`}>
                        ...
                        <div className="flex gap-1">
                          <Button 
                            variant="ghost" 
                            size="icon" 
                            className="h-9 w-9 rounded-xl hover:bg-destructive/10 hover:text-destructive text-muted-foreground/40 transition-premium"
                            onClick={() => {
